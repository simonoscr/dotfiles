    ...
    { name, config, ... }:
    {
      ...
      config = {
        ...
        desktopFile = pkgs.writeTextDir "share/wayland-sessions/${config.fileName}.desktop" ''
          [Desktop Entry]
          Name=${config.name}
          ${lib.optionalString (config.comment != null) "Comment=${config.comment}"}
          Exec=${pkgs.writeShellScript "${config.fileName}-session" config.script}
          Type=Application
        '';
      };
    }
  ...
  config = lib.mkMerge [
    {
      services.greetd.sessionFiles = pkgs.symlinkJoin {
        ...
        passthru.providedSessions = lib.mapAttrsToList (_: session: session.fileName) cfg.sessions;
      };
    }
    (lib.mkIf cfg.nixosIntegration {
      ...
      services.greetd.sessionPath =
        let
          sessionPackage = config.services.xserver.displayManager.sessionData.desktops;
        in
        [
